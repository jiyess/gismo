...
public:
    ...
    T oppoBdryIntegral(const gsBasis<T> *bdryOnedirection, index_t idxBrdy) const {
        ...
        legendreOpts.addInt("quRule", "Quadrature rule used (1) Gauss-Legendre; (2) Gauss-Lobatto; (3) Patch-Rule",
                            gsQuadrature::GaussLegendre);
    ...
    void preComputeBasis() {
        ...
        for (domIt->reset(); domIt->good(); domIt->next()) {
            ...
            gaussIdx.block(0, offset, legendre->numNodes(), legendre->numNodes()) = localIdx.replicate(1,
                                                                                                       legendre->numNodes());
...
//        gsDebug << "Size of allBasisVal = " << m_allBasisVal.rows() << "x" << m_allBasisVal.cols() << "\n";
        ...
        for (index_t gp = 0; gp != m_allBasisVal.cols(); gp++) {
            ...
            if (detJac < m_eps) {
                ...
                for (index_t i = 0; i != hatG.cols(); i++) {
                    ...
                    allG[m_gaussIdx(i, gp) + nAllCtrPts] +=
                            (jacMat(1, 0) * hatG(0, i) - jacMat(0, 0) * hatG(1, i)) * m_gaussWts(gp);
        ...
        for (index_t gp = 0; gp != m_allBasisVal.cols(); gp++) {
            ...
            if (detJac > 1e-10) {
                ...
                parDers_objFun = (2 / detJac * jacMat.transpose() * hatG
                                  ...
                                  + 2 / m_area * (detJac / m_area - 1) * (detJac * jacMat.inverse() * hatG)) *
                                 m_gaussWts(gp);
    ...
    void ImprovePts() {
        ...
        int debug = 1;
        const LBFGS_Optimizer::func_grad_eval func = [&](const std::vector<double> &X, double &F,
                                                         std::vector<double> &G) {
            F = 0;
            std::fill(G.begin(), G.end(), 0);
